...
META = {"name": "sp_writegate", "hypothesis": "fox + one stablegla component", "family": "decay-attention"}
...
class Mixer(nn.Module):
    ...
    def forward(self, x):
        ...
        logwrite = F.logsigmoid(self.write_gate(x) + self.write_bias).transpose(1, 2)
        content = F.softplus((q @ k.transpose(-1, -2)) / math.sqrt(self.dh))
        logits = content.clamp_min(1e-8).log() + bias + logwrite.unsqueeze(-2)

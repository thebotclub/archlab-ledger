...
class Model(nn.Module):
    ...
    def reinitialize_named(self, init_seed):
        """Initialize tensors from (init_seed, parameter name), not RNG order.

        Architectures construct different modules and consume different amounts
        of RNG before Model._init. Per-name streams make every shape-compatible
        common tensor bit-identical so a paired comparison isolates mixer effects.
        """
        for name, param in self.named_parameters():
            ...
            with torch.no_grad():
                if name.endswith("gate.bias") and ".mix." in name and param.ndim == 1:
                    param.fill_(4.0)

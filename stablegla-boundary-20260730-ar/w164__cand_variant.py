...
META = {"name": "onorm_qknorm_3e15", "hypothesis": "conv + bounded selective decay is the minimal pair",
...
class Mixer(nn.Module):
    ...
    def forward(self, x):
        ...
        idx = torch.arange(T, device=x.device); mask = torch.ones(T, T, dtype=torch.bool, device=x.device).tril() & ((idx.unsqueeze(-1) - idx) < 164)

...
while True:
    ...
    if failures:
        ...
        out = {"campaign": R.name, "stage": m["stage"], "claim_eligible": False,
               "reference": "gla 0-for-64 sealed at 3e-3; 1/16 @1.2e16 (campaign u)",
               "arms": arms, "any_gla_transition": total_tr > 0,
               "status": "LR_SWEEP_COMPLETE",
               "completed_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}
        if total_tr > 0:
            subprocess.run(["/home/hani/archlab-operator/notify.sh",
                            f"🚨 NOTABLE: gla transitioned {total_tr}x in LR sweep — "
                            f"gla threshold reachable at 3e15 with tuned LR — reframe gla comparisons. Details in {R.name}/decision.json"],
                           timeout=30)
    else:
        time.sleep(30)
        continue

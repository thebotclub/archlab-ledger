#!/usr/bin/env python3
# =============================================================================
# W1 production runner — campaign spoof-frontier-20260819-sf1 (Lab 4 charter)
#
...
# Production target: (10^12, 10^13].  OEIS cross-reference: A222263.
#
...
#   Largest value ever held in an int64 cell here is sigma(m) for m <= 1e13.
#   Robin/Gronwall bound: sigma(m)/m < e^gamma * ln ln m + 0.6483/ln ln m.
#   For m <= 1e13: ln ln m = ln(29.93) ~= 3.40, so sigma(m)/m < 1.781*3.40
#   + 0.19 < 6.25  =>  sigma(m) < 6.3e13.  int64 max is 9.22e18 — five
...
#   Other quantities: m < 1e13, 2m < 2e13, d = 2m - sigma in (-6.3e13, 2e13),
#   ms/qs entries < 1e13 — all safely inside int64.
#
...
def sigma_direct(m):
    ...
    Independent of the sieve: no shared state, no numpy. For m <= 1e13 trial
    division runs to isqrt(m) <= 3.163e6 (odd steps only) — fine for the rare
...
def run(A, B, workers, state_dir, heartbeat_every, mode):
    ...
    hi_ex = B + 1
    if (hi_ex - lo) // SEG_SPAN + 1 > 400_000:
        log("FATAL: range implies >400k segments — refusing (guard against "
            "mistyped bounds; production (1e12,1e13] needs 360k)", progress)
        sys.exit(2)
    seg_list = []
    L = lo
    idx = 0
...
def main():
    ...
    g.add_argument("--production", action="store_true",
                   help="range (1e12, 1e13], default 48 workers")

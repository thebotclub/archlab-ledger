...
META = {"name": "fox_pro", "hypothesis": "published FoX Pro (norms + KV-shift + output gate, unbounded forget gate, no conv) transitions",
        "family": "foxpro-shell"}
...
class Mixer(nn.Module):
    def __init__(self, d, heads):
        ...
        self.shift_k = nn.Linear(d, 1, bias=False)
        self.shift_v = nn.Linear(d, 1, bias=False)
    ...
    def forward(self, x):
        ...
        k = _tokshift(kt, torch.sigmoid(self.shift_k(x)))
        v = _tokshift(vt, torch.sigmoid(self.shift_v(x)))
